...
export class BranchPartnerComponent implements OnInit {
  ...
  private baseUrlApi : string = "";
   ...
   actions:ActionTypes = new ActionTypes({}, 
    ...
    {isShow:true, icon:'feather-users btn-detail', title:this.translate.instant('add-branch-account')}, {}, {});
   columns = [            
    {
    ...
    },   
    {
      dataField:"phone",
      alignment:"center",
        caption: this.translate.instant('telephone'),
    },
    {
      dataField:"country.title",
      alignment:"center",
        caption: this.translate.instant('country'),
    },
    {
...
openDialogForm(dataModel?:BranchDto) {
  if(dataModel!==undefined && dataModel!==null){
    this.router.navigateByUrl(`/branch/branch-partner/${dataModel.id}`);
  }else{
    this.router.navigateByUrl('/branch/branch-partner/add');
  }
}
    listTitle: string = ''
    ngOnInit() {
        this.listTitle =this.translate.instant('list-of-partner-branches');/// this.tranlate.instant('sarafi.branch.list.header-title')
    this.get();    
  }
  ...
  onDetail(item : BranchDto) {
     this.router.navigateByUrl(`branch/account-partners/${item.id}`)        
  }
  ...
  get(){
    ...
    this.splashScreenService.show();
    this.crudService.getDataUrl(this.baseUrlApi, "GetPartnerBranch").subscribe(res=>{
      this.splashScreenService.hide();
      this.datasource = res ;     
    },
    error =>{
