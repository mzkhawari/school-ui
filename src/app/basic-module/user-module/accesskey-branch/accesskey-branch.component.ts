...
import { AccessKeyBranchFormComponent } from './accesskey-branch-form/accesskey-branch-form.component';

@Component({
  selector: 'accesskey-branch',
  templateUrl: './accesskey-branch.component.html',
})
export class AccessKeyBranchComponent implements OnInit {
  ...
  constructor(
    ...
    this.baseUrlApi = Globals.UrlAccessKeyBranch;
  ...
  columns = [
    ...
    },
    {
      ...
      alignment: "center",
      caption:this.translate.instant('branch')//this.translate.instant('form.province'),
    },
    {
  ...
  openDialogForm(modelAccess?: AccessKeyProvinceDto) {
    ...
    const activeModal = this.modalService.open(AccessKeyBranchFormComponent);
  ...
  OnUpdate(item: AccessKeyProvinceDto) {
    if (!this.accessKeywordService.checkAccessEdit(AccessKeyword.ACCKEY_AccessBranch)) {
      return;
    }
  ...
  OnAdd() {
    if (!this.accessKeywordService.checkAccessAdd(AccessKeyword.ACCKEY_AccessBranch)) {
      return;
    }
    this.openDialogForm();
  }
  ...
  private get() {
    ...
    if (!this.accessKeywordService.checkAccessShow(AccessKeyword.ACCKEY_AccessBranch)) {
      return;
    }

    this.crudService.getInclude(Globals.UrlAccessKeyBranch).subscribe(res => {
