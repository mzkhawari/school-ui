...
import { ClassInfoDto } from 'app/class-module/models/class-info.dto';
  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => ClassInfoFormComponent),
      multi: true
    },
  ]
})
export class ClassInfoFormComponent implements OnInit {
  ...
  constructor(
        ...
        public dialogRef: MatDialogRef<ClassInfoFormComponent>,    
        private toastMessageService: ToastMessageService) {
        this.model = new ClassInfoDto();
      }
  ...
  ngOnInit() {
    ...
    getOption() {
        this.crudService.getDataUrl(Globals.UrlClassInfo, "GetSelectOptions").subscribe(res => {
  ...
  OnSave(model: ClassInfoDto) {
    ...
    if(model.id==0){
      this.crudService.postAdd(Globals.UrlClassInfo, model).subscribe(res => {
        if (res) {
        ...
        this.crudService.putData(Globals.UrlClassInfo, model, model.id).subscribe(res => {
          if (res) {
            this.toastMessageService.showToast(NbToastStatus.SUCCESS, "Success", "SuccessFully is Saved")
            this.dialogRef.close();     
            this.OnRefreshList.next(true);     
