...
import { ClassTimeFormComponent } from './class-time-form/class-time-form.component';
...
export class ClassTimeListComponent implements OnInit,  OnChanges  {
  ...
  constructor(
    ...
    this.model = new ClassTimeDto(0);
    this.baseUrlApi = Globals.UrlClassTime;
  }
  statusTrn:number=0;
...
onDeleteConfirm(item:number) {
      ...
      this.crudService.deleteById(Globals.UrlClassTime, id).subscribe(res=>{
          if(res){
