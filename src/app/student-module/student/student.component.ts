...
export class StudentComponent implements OnInit {
  ...
  constructor(
    ...
    this.baseUrlApi = Globals.UrlStudent;
  }
  statusTrn:number=0;
  ...
  onAddView(dataModel?:any){
    const activeModal = this.modalService.open(StudentFormComponent);
    activeModal.componentInstance.model = Object.assign({},dataModel);        
    activeModal.componentInstance.isViewForm = true; 
  }
  ...
  getData(){
    debugger;
    if(!this.accessKeywordService.checkAccessShow(AccessKeyword.ACCKEY_Student) ){
      return ;
    }
    if(this.searchValue==''){
      this.searchValue = '0';
  ...
  actionList: ActionType[] = [{ isShow: true, title:'حضور فوری', icon: ActionIcon.Tick, level: ActionColor.Success },
                              { isShow: true, title:'نمایش فرم', icon: ActionIcon.View, level: ActionColor.Primary },
                              { isShow: true, title:'ویرایش', icon: ActionIcon.Edit, level: ActionColor.Primary }  ];
  ...
  onActionResult(data: IActionEmitter) {
    ...
    switch (data.index) {      
      case 0:
        ...
        break;
      case 1:
        this.onAddView(data.data);
        break;
      case 2:
        this.onAdd(data.data);
        break;
    }
  }
